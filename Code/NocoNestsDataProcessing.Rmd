...
```{r}
data_dir<-file.path("~/OneDrive - Virginia Tech/Projects-David’s MacBook Air/Maddie/maddie_data/Data/")
figs_dir<-file.path("~/OneDrive - Virginia Tech/Projects-David’s MacBook Air/Maddie/maddie_data/Figures/")
```
...
#Read in files
```{r} 
seqtab.nochim2<-readRDS(file.path(data_dir, "seqtab.nochim.rds"))
sample.names2<-readRDS(file.path(data_dir, "sample.names.rds"))
...
asvannotLong <- read.csv(file.path(data_dir, "maddie_ASVtab.csv"))
...
ggsave(file.path(figs_dir, "noSelfPlot.png"), plot = noSelfPlot, device = 'png', dpi = 300)
```

```{r}

test.enriched <- bind_rows(BHC,SR,MRBD) %>% 
  group_by(Genus, Gender) %>% 
  summarise(observed = sum(Abundance))
female.test <- test.enriched %>% 
  filter(Gender == "F") 
male.test <- test.enriched %>% 
  filter(Gender == "M") 

x <- female.test$observed 
y <- male.test$observed
z <- as.table(rbind(x,y))
dimnames(z) <- list(gender = c("F", "M"),
                    Genus = c("Compostoma","Chrosomus", "Clinostomus", "Luxilus", "Nocomis", "insect"))
